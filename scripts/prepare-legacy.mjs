...
source = source.replace('workbox: {', `workbox: {\n        navigateFallbackDenylist: [new RegExp(${JSON.stringify(deny)}), new RegExp(${JSON.stringify(`^${next}`)})],\n        importScripts: [${JSON.stringify(`${base}codex-bridge.js`)}],`)
writeFileSync(file, source)
writeFileSync(resolve(directory, 'public/codex-bridge.js'), `// Освобождаем новый адрес от старого app-shell только у клиентов старого worker.\nself.addEventListener('activate', event => {\n  event.waitUntil((async () => {\n    await self.clients.claim();\n    const windows = await self.clients.matchAll({ type: 'window' });\n    await Promise.allSettled(windows.filter(client => (new URL(client.url).pathname.startsWith(${JSON.stringify(child)}) || new URL(client.url).pathname.startsWith(${JSON.stringify(next)}))).map(client => client.navigate(client.url)));\n  })());\n});\n`)
